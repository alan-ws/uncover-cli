...
async function fileExists<T>(
  dir: string,
  fileName: T
): Promise<Map<T, boolean>> {
  return new Map().set(
    fileName,
    existsSync(path.join(dir, fileName as string))
  );
}
...
async function handleGitIgnore(
  value: Map<"uncover.json" | ".gitignore" | ".vercelignore", boolean>,
  args: string
) {
  if (!value.get(".gitignore")) {
    ...
    appendFileSync(path.join(args, ".gitignore"), "\n#uncover\nuncover.json");
  }
}

async function handleVercelIgnore(
  value: Map<"uncover.json" | ".gitignore" | ".vercelignore", boolean>,
  args: string
) {
...
function getProjectId() {
  ...
  const file = JSON.parse(
    readFileSync(path.join(process.cwd(), ".vercel", "project.json"), {
      encoding: "utf-8",
    })
  );
...
export function link() {
  ...
  const projectIdOption = new Option(
    "-p --project-id [id]",
    "vercel project id"
  );
  const patchOption = new Option(
    "--patch",
    "patch .vercelignore and .gitignore with uncover.json"
  );
  const directoryOption = new Option(
    "-d --directory [directory]",
    "absolute path to vercel project"
  );
  ...
  directoryOption.argParser(async (args) => {
    ...
    if ((await fileExists(args, "uncover.json")).get("uncover.json")) {
      ...
      console.log(
        "Ensure uncover.json is added to .gitignore and .vercelignore"
      );
      console.log(
        "Run uncover link --patch to add uncover.json to those files"
      );
      exit(0);
    }
    ...
    const files: Map<
      "uncover.json" | ".gitignore" | ".vercelignore",
      boolean
    >[] = await Promise.all([
